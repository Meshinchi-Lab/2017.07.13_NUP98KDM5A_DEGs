...
```{r}
KDM1A <- TPM[c("ENSG00000004487","ENSG00000165702"),] %>% 
  t() %>% 
  as.data.frame() %>% 
  rownames_to_column("USI") %>% 
  dplyr::select(USI, KDM1A_TPM=ENSG00000004487.16, 
                GFI1B_TPM=ENSG00000165702.14) %>%
  mutate(log2_KDM1A_TPM=log2(KDM1A_TPM +1), 
         log2_GFI1B_TPM=log2(GFI1B_TPM + 1)) %>%
  
  left_join(., dplyr::select(manifest, USI, Reg.),
            by="USI") %>% 
  left_join(., merged %>% mutate(Reg.=as.character(Reg.)),
            by="Reg.") %>%
  dplyr::select(USI=USI.x, everything()) %>%
  left_join(., dplyr::select(all_fusions, USI, FOI), 
            by="USI") %>% 
...
```{r}
quantile(KDM1A[KDM1A$Group=="AML","GFI1B_TPM"])
quantile(KDM1A[KDM1A$Group=="NBM","GFI1B_TPM"])
```


```{r}
hist(KDM1A[KDM1A$Group=="AML","GFI1B_TPM"], breaks = seq(0, max(KDM1A$GFI1B_TPM)+5, by=2.5),
     col="turquoise", main = "GFI1B  Expression in \n Pediatric AML and NBM Controls")
hist(KDM1A[KDM1A$Group=="NBM","GFI1B_TPM"], breaks = seq(0, max(KDM1A$GFI1B_TPM)+5, by=5), col="darkgrey", main = NULL, add=TRUE)
abline(v=median(KDM1A$GFI1B_TPM), col="black", lwd=4,lty=2)
legend(x=100, y=200, legend = c("AML", "NBM"), col=c("turquoise","darkgrey"), pch=15)
```
...
```{r}
w.fus.GFI1B <- ggbarplot(KDM1A, x="USI", y="GFI1B_TPM",
          fill="Rare.Fusions",
          color="Rare.Fusions",
          palette = "jco",
          sort.val = "asc",
...
```{r}
v.fus.GFI1B <- ggviolin(KDM1A, x="Rare.Fusions", y="log2_GFI1B_TPM",
          fill="Rare.Fusions",
          color="darkgrey",
          palette = "jco",
          sort.val = "asc",
...
```{r}
df.GFI1B <- dplyr::select(KDM1A,log2_GFI1B_TPM,GFI1B_TPM, Rare.Fusions) %>% 
  filter(Rare.Fusions != "Unknown") %>% 
  group_by(Rare.Fusions) %>% 
  summarize(median=median(log2_GFI1B_TPM), 
            median.norm=median(GFI1B_TPM)) %>% 
  arrange(desc(median))
